...
const frameLibraryService_1 = require("../services/frameLibraryService");
// Cache asset loading to prevent reloading the same images
const assetCache = new Map();
class VideoProcessor {
    constructor() {
        var _a;
        this.frameLibraryInitialized = false;
        // Vertical dimensions for TikTok (9:16 aspect ratio)
        this.width = config_1.default.video.width || 720;
        this.height = config_1.default.video.height || 1280;
        ...
        this.frameLibrary = new frameLibraryService_1.FrameLibraryService();
    }
    /**
    ...
    initialize() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.frameLibraryInitialized)
                return;
            logger_1.default.info('Initializing VideoProcessor...');
            // Preload assets
            ...
            yield this.frameLibrary.initialize(assets, true); // Pass true to use more rotation values
            this.frameLibraryInitialized = true;
            logger_1.default.info('VideoProcessor initialized with pre-rendered frames');
        });
    }
    /**
    ...
    generateFrameSequence(audioDuration_1, audioPath_1, contentText_1) {
        return __awaiter(this, arguments, void 0, function* (audioDuration, audioPath, contentText, isShortFormat = false, newsTitle, newsImagePath) {
            try {
                ...
                logger_1.default.info(`Video format: ${isShort ? 'SHORT' : 'STANDARD'} (${audioDuration}s)`);
                // Determine content type if text is provided
                const contentType = contentText ? this.determineContentType(contentText) : 'WEIRD';
                // Create output directory
                const outputDir = path.join(config_1.default.paths.frames, `seq_${Date.now()}`);
                ...
                if (newsImagePath && fs.existsSync(newsImagePath)) {
                    try {
                        newsImage = yield (0, canvas_1.loadImage)(newsImagePath);
                        logger_1.default.info(`Loaded news image: ${newsImagePath}`);
                    }
                    catch (err) {
                        logger_1.default.warn(`Failed to load news image: ${err}`);
                    }
                }
                ...
                const sceneCanvas = (0, canvas_1.createCanvas)(this.width, this.height);
                const sceneCtx = sceneCanvas.getContext('2d');
                // Pre-draw static background and studio elements
                this.drawTopSection(sceneCtx, newsTitle || contentType);
                // Studio background
                sceneCtx.drawImage(assets.background, 0, this.height * 0.2, // Position at 20% height
                this.width, this.height * 0.5 // Take up 50% of height
                );
                // Desk
                sceneCtx.drawImage(assets.desk, 0, this.height * 0.55, // Position just above bottom section
                this.width, this.height * 0.15);
                // Props
                if (assets.props.monitor) {
                    sceneCtx.drawImage(assets.props.monitor, this.width * 0.7, this.height * 0.4, this.width * 0.15, this.height * 0.08);
                }
                if (assets.props.mug) {
                    sceneCtx.drawImage(assets.props.mug, this.width * 0.2, this.height * 0.55, this.width * 0.06, this.height * 0.03);
                }
                ...
                for (let i = 0; i < totalFrames; i++) {
                    ...
                    if (i % 30 === 0 || i === totalFrames - 1) {
                        ...
                        logger_1.default.info(`Generated ${i}/${totalFrames} frames (${percentComplete}%)`);
                    }
                }
                ...
                logger_1.default.info(`Frame generation complete: ${frames.length} frames in ${elapsedSeconds.toFixed(1)}s (${framesPerSecond.toFixed(1)} fps)`);
                return {
                    directory: outputDir,
                    frames,
     ...
     */
    drawCharacterDirectly(ctx, assets, mouthState, eyeState, headRotation) {
        // Character position
        ...
        switch (eyeState) {
            case 'squint':
                eyeImage = assets.eyes.squint;
                break;
            case 'wide':
                eyeImage = assets.eyes.wide;
                break;
            case 'rolling':
                eyeImage = assets.eyes.rolling;
                break;
            case 'wink':
                eyeImage = assets.eyes.wink;
                break;
        ...
        switch (mouthState) {
            case 'halfOpen':
                mouthImage = assets.mouths.halfOpen;
                break;
            case 'open':
                mouthImage = assets.mouths.open;
                break;
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    if (this.width > this.height) {
                        logger_1.default.warn(`Warning: Video dimensions appear to be landscape (${this.width}x${this.height}), should be portrait`);
                    }
     ...
     */
    drawBottomSection(ctx, newsImage, contentType) {
        // Divider bar
        ...
        if (newsImage) {
            ...
            ctx.drawImage(newsImage, x + offsetX, y + offsetY, drawWidth, drawHeight);
    ...
    analyzeAudio(audioPath) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const durationOutput = yield this.getAudioDuration(audioPath);
                ...
                logger_1.default.warn(`Audio analysis failed, using fallback: ${error}`);
    ...
    getAudioDuration(audioPath) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve) => {
                ...
                const process = (0, child_process_1.spawn)(ffmpeg_static_1.default, durationArgs);
                let output = '';
    ...
    estimateDurationFromFileSize(audioPath) {
     ...
     */
    saveFrame(canvas, frameNumber, outputDir) {
    ...
    createDefaultDesk() {
        const canvas = (0, canvas_1.createCanvas)(this.width, this.height * 0.4);
        const ctx = canvas.getContext('2d');
        // Simple desk
    ...
    createDefaultBody() {
        const canvas = (0, canvas_1.createCanvas)(300, 360);
        const ctx = canvas.getContext('2d');
        // Simple potato body
    ...
    createDefaultMouth(state) {
        const canvas = (0, canvas_1.createCanvas)(120, 60);
        const ctx = canvas.getContext('2d');
    ...
    createDefaultEyes(state) {
        const canvas = (0, canvas_1.createCanvas)(200, 80);
        const ctx = canvas.getContext('2d');
        ctx.fillStyle = '#ffffff';
        // Left eye
        ctx.beginPath();
     ...
     */
    determineMouthState(amplitude, prevAmplitude, timePosition) {
     ...
     */
    determineEyeState(timePosition, amplitude) {
     ...
     */
    calculateHeadMovement(timePosition, amplitude) {
