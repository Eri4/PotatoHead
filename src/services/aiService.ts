...
Keep under 40-50 words total for maximum engagement. This should be ~15-20 seconds when read aloud.`
            ...
            logger.info(`Generated content with ID: ${content.id}, image terms: ${imageTerms.join(', ')}`);
            return content;
        } catch (error) {
...
For the COMMENTARY:
- VIRAL HOOK: Choose the appropriate style based on your content type determination, try to not use the same hook formula too many times:
  ...
  * Use a more dramatic, intrigued tone throughout
  ...
  IF YOU NEED A NEW HOOK: Create one that follows the same pattern (3-7 words with ONE emphasized word) but uses different phrasing than previous hooks. Focus on the main emotion of the story (surprise, mystery, humor, etc).
...
Respond AS PotatoHead. Include ALL THREE sections (COMMENTARY, IMAGE_SEARCH_TERMS, CONTENT_TYPE) in your response.`;
    ...
    private parseResponse(text: string): {
        commentary: string;
        imageTerms: string[];
        contentType: 'mystery' | 'weird' | 'unexplained' | 'cryptid' | 'paranormal' | 'funny' | null
    } {
        ...
        if (contentTypeMatch && contentTypeMatch[1]) {
            contentType = contentTypeMatch[1].toLowerCase() as any;
        }
     ...
     */
    private cleanBrackets(text: string): string {
